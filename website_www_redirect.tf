...
resource "aws_cloudfront_distribution" "www_website" { # tfsec:ignore:AWS045
  ...
  default_cache_behavior {
    ...
    target_origin_id         = local.www_website_bucket_name
  ...
  origin {
    domain_name = aws_s3_bucket.www_website.bucket_regional_domain_name
    origin_id   = local.www_website_bucket_name
    s3_origin_config {
